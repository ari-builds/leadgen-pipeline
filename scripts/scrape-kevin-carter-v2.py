...
def extract_email(html):
    ...
    if match:
        ...
        junk = ['user@', 'you@', 'name@', 'your@', 'business@', 'email@', 'noreply', 'support@', 'image', 'example@', 'm-header-logo']
...
SKIP_DOMAINS = ['yelp.com','bbb.org','facebook.com','yellowpages.com','tripadvisor.com','foursquare.com','thumbtack.com','homeadvisor.com','angi.com','houzz.com','bark.com','expertise.com','porch.com','nextdoor.com','linkedin.com','twitter.com','instagram.com','wikipedia.org','reddit.com','duckduckgo.com','google.com','bing.com','mapquest.com','superpages.com','manta.com','indeed.com','glassdoor.com','youtube.com','fresha.com','vagaro.com','squire.com','procore.com','lessons.com','findglocal.com']

def save_lead(c, conn, name, url, industry, email, phone, fb, ig, li, location, notes, score, client_id):
    try:
        c.execute('''INSERT INTO leads 
            (company_name, website_url, industry, contact_name, contact_email, contact_phone,
             contact_linkedin, location, notes, score, status, source_url, 
             contact_facebook, contact_instagram,
             created_at, updated_at) 
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'),datetime('now'))''',
            (name, url, industry, '', email, phone, li, location, notes, score, 'new', url, fb, ig))
        lead_id = c.lastrowid
        c.execute('INSERT INTO client_leads (client_id, lead_id, assigned_at) VALUES (?, ?, datetime(\'now\'))', (client_id, lead_id))
        conn.commit()
        return True
    except Exception as e:
        print(f'    [ERROR] {e}')
...
def scrape_carter():
    ...
    queries = [
        'restaurant Tyngsborough MA', 'pizza shop Lowell MA', 'Mexican restaurant Tewksbury MA',
        'cafe Billerica MA', 'bakery Chelmsford MA', 'food truck Lowell MA',
        'breakfast restaurant Dracut MA', 'plumber Tyngsborough MA', 'electrician Tewksbury MA',
        'HVAC contractor Chelmsford MA', 'roofing company Dracut MA', 'landscaping Billerica MA',
        'general contractor Lowell MA', 'painting contractor Andover MA', 'hair salon Tyngsborough MA',
        'nail salon Tewksbury MA', 'barbershop Chelmsford MA', 'beauty salon Lowell MA',
        'spa Billerica MA', 'gym Tyngsborough MA', 'yoga studio Tewksbury MA',
        'personal trainer Lowell MA', 'boutique Lowell MA', 'gift shop Andover MA',
        'flower shop Chelmsford MA', 'pet store Tewksbury MA', 'auto repair Tyngsborough MA',
        'dentist Tewksbury MA', 'chiropractor Chelmsford MA', 'veterinary Lowell MA',
        'accountant Billerica MA', 'real estate agent Tyngsborough MA',
        'restaurant Nashua NH', 'contractor Nashua NH', 'salon Nashua NH',
        'gym Nashua NH', 'auto repair Nashua NH',
    ]
    ...
    for i, query in enumerate(queries):
        ...
        for result in results[:4]:
            ...
            if any(x in url for x in ['webdesign', 'web-design', 'squarespace.com', 'wix.com', 'shopify.com', 'wordpress.com', 'godaddy.com']): continue
            if any(x in result['title'].lower() for x in ['web design', 'website builder', 'website template', 'how to build', 'signs your website', 'best website']): continue
            ...
            elif re.search(r'contractor|construction|plumb|electric|hvac|roofing|landscape|painting', text): industry = 'Contractor'
            ...
            elif re.search(r'boutique|gift shop|flower|pet store|retail', text): industry = 'Retail'
            ...
            if save_lead(c, conn, name, result['url'], industry, email, phone, fb, ig, li, location, notes, score, 7):
                total_saved += 1
    ...
    total = c.fetchone()[0]
    print(f'\nCARTER: Saved {total_saved} new, {total} total')
    conn.close()
...
def scrape_kevin():
    ...
    queries = [
        'cemetery association Washington state', 'funeral home Yakima WA',
        'crematory Yakima WA', 'monument dealer Yakima WA',
        'funeral director Yakima WA', 'cemetery manager Washington',
        'funeral home Ellensburg WA', 'cemetery association Ellensburg WA',
        'funeral home Selah WA', 'funeral home Union Gap WA',
        'crematory Yakima County', 'monument company Washington state',
        'headstone dealer Washington', 'funeral service Toppenish WA',
        'funeral home Wapato WA', 'monument engraver Yakima',
        'memorial park Yakima', 'headstone cleaning service near me',
        'gravestone repair near me', 'cemetery cleaning service Washington',
        'monument restoration Pacific Northwest', 'grave cleaning service WA',
        'tombstone cleaning Washington state', 'headstone polishing service',
        'cemetery maintenance service', 'grave care service Washington',
        'memorial cleaning near me', 'headstone restoration near me',
        'monument builders Washington', 'cemetery developers Northwest',
        'memorial products Washington state', 'funeral supplies Yakima',
        'cemetery equipment Washington', 'headstone wholesale Washington',
        'cemetery maintenance products', 'gravestone sealant Washington',
        'monument cleaning supplies', 'funeral home Grandview WA',
        'cemetery Prosser WA', 'funeral home Zillah WA',
        'cemetery Sunnyside WA', 'monument Richland WA',
        'funeral home Kennewick WA', 'crematory Tri-Cities WA',
        'monument Pasco WA', 'funeral services Benton City WA',
        'cemetery Walla Walla WA', 'funeral home Walla Walla WA',
        'monument Spokane WA', 'cemetery Spokane WA',
    ]
    ...
    for i, query in enumerate(queries):
        ...
        for result in results[:4]:
            ...
            seen.add(url)
            if any(d in url for d in SKIP_DOMAINS): continue
            if any(x in url for x in ['article', 'blog', 'news', '.pdf', 'wikipedia', 'findagrave', 'obituary', 'legacy.com']): continue
            ...
            wa_cities = ['Yakima', 'Ellensburg', 'Selah', 'Union Gap', 'Toppenish', 'Wapato', 'Naches', 'Moxee', 'Grandview', 'Prosser', 'Sunnyside', 'Richland', 'Kennewick', 'Pasco', 'Benton City', 'Zillah', 'Granger', 'Spokane', 'Walla Walla']
            location_match = None
            ...
            if save_lead(c, conn, name, result['url'], industry, email, phone, fb, ig, li, location_match, notes, score, 2):
                total_saved += 1
                print(f'    [SAVED] {name} | {email or "no email"} | {phone or "no phone"}')
    ...
    total = c.fetchone()[0]
    print(f'\nKEVIN: Saved {total_saved} new, {total} total')
    conn.close()
